...
class TestGestorDeReclamos(unittest.TestCase):
    ...
    def setUp(self):
        self.__repo_mock = MagicMock()
        self.__gestor = GestorDeReclamos(self.__repo_mock)
    ...
    def test_obtener_tiempos_por_departamento_devuelve_tiempos_correctos(self):
        ...
        self.__repo_mock.obtener_registros_por_filtro.return_value = [reclamo1, reclamo2, reclamo3]

        # Instanciamos el SUT con el mock
        gestor = GestorDeReclamos(self.__repo_mock)
        ...
        self.assertEqual(tiempos_estimados, [5])  # Solo los tiempos no nulos
        self.assertEqual(tiempos_ocupados, [6])   # Solo los tiempos no nulos
        # Verificamos que el método del mock fue llamado con los argumentos correctos
        self.__repo_mock.obtener_registros_por_filtro.assert_called_once_with('clasificacion', 'soporte informático')
    ...
    def test_creación_reclamo_exitoso(self):
        ...
        self.__repo_mock.obtener_todos_los_registros.return_value = []
        self.__gestor._GestorDeReclamos__clasificador = clasificador_mock
        ...
        with patch('modules.gestorReclamos.datetime') as mock_datetime:
            ...
            # Act
            self.__gestor.crear_reclamo(contenido, id_usuario)

            # Assert
            clasificador_mock.clasificar_reclamo.assert_called_once_with(contenido)
            self.__repo_mock.guardar_registro.assert_called_once()
            args, _ = self.__repo_mock.guardar_registro.call_args
            ...
            self.assertEqual(reclamo_guardado.tiempo_estimado, None)
            self.assertEqual(reclamo_guardado.tiempo_ocupado, None)

            self.assertEqual(self.__gestor._GestorDeReclamos__numero_reclamos, 1)
    ...
    def test_obtener_tiempos_devuelve_listas_correctas(self):
        ...
        self.__repo_mock.obtener_todos_los_registros.return_value = [reclamo1, reclamo2, reclamo3]

        # Act
        tiempos_estimados, tiempos_ocupados = self.__gestor.obtener_tiempos()

        # Assert
        # Esperamos: tiempos estimados = [5, 3, 4], tiempos ocupados = [None, 2, None]
        self.assertEqual(tiempos_estimados, [5])
        self.assertEqual(tiempos_ocupados, [2])
        # Verificamos que se llamó al método del repositorio
        self.assertEqual(self.__repo_mock.obtener_todos_los_registros.call_count,2)
